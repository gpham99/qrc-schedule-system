...
const TimeWindow = () => {
  ...
  useEffect(() => {
    async function fetchBlock() {
      try {
        ...
        const res = await fetch("https://44.228.177.192/api/get_block");
  ...
  useEffect(() => {
    async function fetchIsOpen() {
      try {
        ...
        const res = await fetch("https://44.228.177.192/api/is_open");
        const data = await res.json();
  ...
  const update = async () => {
    try {
      setSubmitting(true);
      const res = await fetch("https://44.228.177.192/api/open_schedule", {
      method: "POST",
