...
const TutorInfo = () => {
  ...
  useEffect(() => {
      ...
      async function fetchTutors() {
        try {
          setLoading(true);
          const res = await fetch("https://44.230.115.148/api/get_tutors_information", requestOptions);
  ...
  const submitChange = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch("https://44.230.115.148/api/set_tutors_information", requestOptions);
      if (res.status === 200) {
